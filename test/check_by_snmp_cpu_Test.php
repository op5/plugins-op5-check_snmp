...
class Check_Snmp_Cpu_Test extends test_helper
{
	 ...
	 */
	public function test_cpu_default_OK($conn_args) {
		...
		$this->assertCommand($conn_args, "", array(
			...
			"UNKNOWN: No difference in SNMP counters since first " .
			"execution, please re-run the plugin in a few seconds"
		), 3);
		// Add some new values
		$this->assertCommand($conn_args, "", array(
			...
			"OK: total CPU usage at 0.00% | " .
			"total=0.00%;100.0;100.0 user=0.00% system=0.00% " .
			"iowait=0.00% kernel=0.00% steal=0.00% nice=0.00% idle=100.00%"
		), 0);
		...
		/*
		 * Update values, and verify that it saves the previous state,
		 * and uses the new values.
		 */
		$this->assertCommand($conn_args, "", array(
			"1.3.6.1.4.1.2021.11.53.0" => array(65,103504746+1),
			"1.3.6.1.4.1.2021.11.50.0" => array(65,252827+1)
		), array(
			"OK: total CPU usage at 100.00% | " .
			"total=100.00%;100.0;100.0 user=100.00% system=0.00% " .
			"iowait=0.00% kernel=0.00% steal=0.00% nice=0.00% idle=0.00%"
		), 0);
	}
	 ...
	 * @dataProvider snmpArgsProvider
	 */
	public function test_cpu_default_initial_WARNING($conn_args) {
		$this->assertCommand($conn_args, " -w 55: -c 90", array(
		), array(
			"UNKNOWN: No previous state, initializing database. Re-run the plugin"
		), 3);
		$this->assertCommand($conn_args, " -w 55: -c 90", array(
			...
			"WARNING: total CPU usage at 50.00% | " .
			"total=50.00%;55:;90 user=50.00% system=0.00% " .
			"iowait=0.00% kernel=0.00% steal=0.00% nice=0.00% idle=50.00%"
		), 1);
	}
	 ...
	 * @dataProvider snmpArgsProvider
	 */
	public function test_cpu_default_initial_CRITICAL($conn_args) {
		$this->assertCommand($conn_args, " -w 55: -c~:90", array(
		), array(
			"UNKNOWN: No previous state, initializing database. Re-run the plugin"
		), 3);
		$this->assertCommand($conn_args, " -w 55: -c~:90", array(
			...
			"CRITICAL: total CPU usage at 100.00% | " .
			"total=100.00%;55:;~:90 user=100.00% system=0.00% " .
			"iowait=0.00% kernel=0.00% steal=0.00% nice=0.00% idle=0.00%"
		), 2);
	}
	 ...
	 * @dataProvider snmpArgsProvider
	 */
	public function test_cpu_perfdata_same_amounts_of_ticks_initial_run_OK($conn_args) {
		$this->assertCommand($conn_args, "-w 50:90 -c 80:95", array(
		), array(
			"UNKNOWN: No previous state, initializing database. Re-run the plugin"
		), 3);
		$this->assertCommand($conn_args, "-w 50:90 -c 80:95", array(
			...
			"OK: total CPU usage at 83.33% | " .
			"total=83.33%;50:90;80:95 user=16.67% system=16.67% " .
			"iowait=16.67% kernel=0.00% steal=16.67% nice=16.67% idle=16.67%"
		), 0);
	}
	 ...
	 * @dataProvider snmpArgsProvider
	 */
	public function test_cpu_perfdata_initial_run_OK($conn_args) {
		$this->assertCommand($conn_args, "-w 50:95 -c 80:95", array(
		), array(
			"UNKNOWN: No previous state, initializing database. Re-run the plugin"
		), 3);
		$this->assertCommand($conn_args, "-w 50:95 -c 80:95", array(
			...
			"OK: total CPU usage at 80.95% | " .
			"total=80.95%;50:95;80:95 user=4.76% system=14.29% " .
			"iowait=23.81% kernel=0.00% steal=28.57% nice=9.52% idle=19.05%"
		), 0);
	}
	 ...
	 * @dataProvider snmpArgsProvider
	 */
	public function test_cpu_initial_run_UNKNOWN($conn_args) {
		$this->assertCommand($conn_args, "-T user -w 90 -c 95", array(
		), array(
			"UNKNOWN: No previous state, initializing database. Re-run the plugin"
		), 3);
		$this->assertCommand($conn_args, "-T user -w 90 -c 95", array(
		...
		$this->assertCommand($conn_args, "-T user -w 90 -c 95", array(
			...
			"OK: user CPU usage at 50.00% | " .
			"total=50.00% user=50.00%;90;95 system=0.00% " .
			"iowait=0.00% kernel=0.00% steal=0.00% nice=0.00% idle=50.00%"
		), 0);
		$this->assertCommand($conn_args, "-T user -w 90 -c 95", array(
			...
			"OK: user CPU usage at 25.00% | " .
			"total=50.00% user=25.00%;90;95 system=0.00% " .
			"iowait=0.00% kernel=0.00% steal=0.00% nice=25.00% idle=50.00%"
		), 0);
	}
	...
	public function test_no_arguments() {
		$this->assertCommand("", "", array(
			...
			'Usage:',
			'check_by_snmp_cpu [-w <warn_range>] [-c <crit_range>] [-T <type>]',
			$this->snmp_usage
		), 3);
	}
}
