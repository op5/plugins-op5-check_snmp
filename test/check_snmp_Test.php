<?php
class Check_Snmp_Test extends PHPUnit_Framework_TestCase {

	private static $snmpsimroot = "/tmp/check_snmp_test/";
 ...
 */
	public function test_percent_storage_used_OK() {
		$this->assertCommand("-H @endpoint@ -C mycommunity -f -i 31 -w 50 -c 75", array(
		), array(
			"OK: 32% of storage used |'/'=32%;50;75"
		), 0);
	}
	public function test_percent_storage_used_WARNING() {
		$this->assertCommand("-H @endpoint@ -C mycommunity -f -i 31 -w 25 -c 75", array(
		), array(
			"WARNING: 32% of storage used |'/'=32%;25;75"
		), 1);
	}
	public function test_percent_storage_used_CRITICAL() {
		$this->assertCommand("-H @endpoint@ -C mycommunity -f -i 31 -w 25 -c 30", array(
		), array(
